...
export default function NewQuestion() {
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-3xl font-bold text-white mb-8">Add Question</h1>
      ...
      <div className="space-y-5">
        ...
        <div>
          <label className="block text-gray-400 text-sm mb-2">Solution SQL (hidden from students)</label>
          <textarea value={form.solution_sql} onChange={e => setForm({...form, solution_sql: e.target.value})}
            rows={5} placeholder="SELECT ..."
            className="w-full bg-gray-900 border border-gray-600 rounded-lg px-4 py-3 text-white font-mono text-sm focus:outline-none focus:border-blue-500" />
        </div>

        <div className="flex items-center gap-3">
          <input type="checkbox" id="order" checked={form.order_matters}
            onChange={e => setForm({...form, order_matters: e.target.checked})}
            className="w-4 h-4" />
          <label htmlFor="order" className="text-gray-400 text-sm">Order matters (grading will be order-sensitive)</label>
        </div>
