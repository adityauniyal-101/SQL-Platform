...
export async function POST(req: NextRequest) {
  ...
  try {
    for (const question of questions) {
      ...
      if (!sql) {
        await tx.execute(updateAnswerSql, [parsed.data.submission_id, question.id, null, 0]);
        continue;
      }
      ...
      if (isCorrect) score++;
      await tx.execute(updateAnswerSql, [parsed.data.submission_id, question.id, sql, isCorrect ? 1 : 0]);
    }
